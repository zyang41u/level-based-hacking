...
# Import lbh module to ensure it's available in multiprocessing workers
try:
    import lbh  # noqa
except ImportError:
    # Try adding the epymarl directory to the path so lbh can be imported as a package
    # __file__ is src/runners/parallel_runner.py, so go up two levels to epymarl
    epymarl_dir = os.path.dirname(os.path.dirname(os.path.dirname(__file__)))
    if os.path.exists(epymarl_dir) and epymarl_dir not in sys.path:
        sys.path.insert(0, epymarl_dir)
    try:
        import lbh  # noqa
    except ImportError:
        pass  # lbh may not be installed, but that's okay if not using lbh environments
...
def env_worker(remote, env_fn):
    # Import lbh module to ensure it's available in this worker process
    # This is critical because multiprocessing workers start with a fresh Python interpreter
    try:
        import lbh  # noqa
    except ImportError:
        # Try adding the epymarl directory to the path so lbh can be imported as a package
