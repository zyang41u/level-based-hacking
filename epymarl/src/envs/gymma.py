...
class GymmaWrapper(MultiAgentEnv):
    def __init__(
        ...
        **kwargs,
        ...
        # Import lbh module if the key uses lbh namespace to ensure it's available
        # This is critical for multiprocessing workers where the module might not be imported
        if isinstance(key, str) and key.startswith("lbh:"):
            try:
                import lbh  # noqa
            except ImportError:
                pass  # lbh may not be installed, but that's okay if not using lbh environments
